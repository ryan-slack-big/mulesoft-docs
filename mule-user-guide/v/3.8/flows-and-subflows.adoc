...
[cols="10,40,50"]
|===
|*1* |*Subflow* |A subflow processes messages *synchronously*, relative to the flow that triggered its execution. It has no processing strategy or exception strategy of its own, and is treated the same as if its member components were directly added to the triggering flow. A subflow can only be triggred from a flow-ref, and can not have an inbound endpoint.
